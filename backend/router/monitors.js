...
router.get('/get-data', async(req, res) => {
    const getData = await Monitor.find()
    res.send(getData)
})

router.get('/get-data-byId/:id', async(req, res) => {
    try {
        const getData = await Monitor.find({_id: req.params.id})
...
router.post('/delete-data' , async (req , res) => {
    const data = await Monitor.deleteOne({_id: req.body.id})
...
router.post('/update-data/:id', async(req, res) => {
    try {
        const getData = await Monitor.updateOne({_id: req.params.id} , {
            $set: {
