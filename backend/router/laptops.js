...
router.get('/get-data', async(req, res) => {
    const getData = await Laptop.find()
    res.send(getData)
})

router.get('/get-data-byId/:id', async(req, res) => {
    try {
        const getData = await Laptop.find({_id: req.params.id})
...
router.post('/delete-data' , async (req , res) => {
    const data = await Laptop.deleteOne({_id: req.body.id})
...
router.post('/update-data/:id', async(req, res) => {
    try {
        const getData = await Laptop.updateOne({_id: req.params.id} , {
            $set: {
